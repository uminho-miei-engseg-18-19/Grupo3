...
def parseArgs():
    if (len(sys.argv) != 2) and (sys.argv[3] == '-msg') and (sys.argv[5] == '-sDash') and (sys.argv[7] == '-f')
        eccPublicKeyPath = sys.argv[1]
        data = sys.argv[3]
        signature = sys.argv[5]
        with open('ficheiro_do_requerente', 'r') as f:
            blindComponents = f.readline()[0]
            pRComponents = f.readline()[1]
        return data, signature, blindComponents, pRComponents, eccPublicKeyPath
