...
def parseArgs():
    if (len(sys.argv) > 1) and (sys.argv[1] == '-msg') and (sys.argv[3] == '-RDash'):
        data = sys.argv[2] 
        pRDashComponents = sys.argv[4]
        return data, pRDashComponents
    else:
        print ('error')
...
def showResults(errorCode, result):
    ...
    if (errorCode is None):
        ...
        with open('ficheiro_do_requerente', 'w') as f:
            f.write("Blind components: %s \n" % blindComponents)
            f.write("pRComponents: %s " % pRComponents)
